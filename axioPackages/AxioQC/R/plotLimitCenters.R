plotLimitCenters <- function( data , pathRoot = "./" , level = "100ng" , type = "quarterly" , main = "Interval Center by Quarter" , xlab = "Quarter" , ylab = "Interval Center" , cumulative = FALSE , PNG = FALSE )
{
  limitWidth <- data.frame( width = unlist( lapply( data[["bootPercentiles"]] , function( x ) return( median( x[1:2] ) ) ) ) , quarter = names( data[["bootPercentiles"]] ) , stringsAsFactors = FALSE )
  ...
  if ( PNG )
  {
    png( paste( pathRoot , paste( ifelse( cumulative , "cumulative_intervalcenter" , "intervalcenter" ) , level , type , "png" , sep = "." ) , sep = "/" ) , width = 9.75 , height = 5.8 , units = "in" , res = 125 )
  }
